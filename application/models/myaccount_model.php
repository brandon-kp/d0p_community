...
class Myaccount_model extends CI_Model {
	...
	public function edit_profile($id,$login_session,$name,$location,$signature,$side_title,$side_table,$main_table,$extra_title1,$extra_table1,$extra_title2,$extra_table2)
	{
		$this->db
			...
			->where('id',$id)
			->update('user_profile',array('name'=>$name,
										  'location'=>$location,
										  'signature'=>$signature,
	...
	public function pending_buddies($id)
	{
		$query = $this->db
					...
					->select('user_profile.photo,user_profile.name,user_relationships.user_1,user_profile.photo,user_relationships.buddy_msg')
					->get_where('user_relationships',array('user_2'=>$id, 'buddy_status'=>'0','block'=>NULL))
					->result_array();
		return $query;
	}
	...
	public function approve_buddy($user_id, $friend_id)
	{
		$this->db
			...
			->update('user_relationships',array('buddy_status'=>'1',
												'buddy'=>'1',
												'buddy_msg'=>''));
	}
